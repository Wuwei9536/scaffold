...
const judgeMouthWeek = (month, date, day, maxdate) => { // 月份 日期 星期 该月最大天数
    ...
    date -= 7;
    let subTimeShow = judgeMouthWeek(month, date, day, 100);
...
const initTimeShow = (year, month, date, quarter) => { // month为moment().month()+1 // 年 月 日 季度
    ...
    if (month > endMonth) {
        let timeShowThree = judgeMouthWeek(specMonth[2], dayNumber[2], moment(lastDay[2], "YYYY-MM-DD").day(), dayNumber[2]);
        let timeShowTwo = judgeMouthWeek(specMonth[1], dayNumber[1], moment(lastDay[1], "YYYY-MM-DD").day(), dayNumber[1]);
        let timeShowOne = judgeMouthWeek(specMonth[0], dayNumber[0], moment(lastDay[0], "YYYY-MM-DD").day(), dayNumber[0]);
        ...
        let timeShowThree = judgeMouthWeek(specMonth[2], date, day, dayNumber[2]);
        let timeShowTwo = judgeMouthWeek(specMonth[1], dayNumber[1], moment(lastDay[1], "YYYY-MM-DD").day(), dayNumber[1]);
        let timeShowOne = judgeMouthWeek(specMonth[0], dayNumber[0], moment(lastDay[0], "YYYY-MM-DD").day(), dayNumber[0]);
        let result = timeShowThree.concat(timeShowTwo, timeShowOne);
        return result;
    } else if (month % 3 === 2) {
        let timeShowTwo = judgeMouthWeek(specMonth[1], date, day, dayNumber[1]);
        let timeShowOne = judgeMouthWeek(specMonth[0], dayNumber[0], moment(lastDay[0], "YYYY-MM-DD").day(), dayNumber[0]);
        let result = timeShowTwo.concat(timeShowOne);
        return result;
    } else if (month % 3 === 1) {
        let timeShowOne = judgeMouthWeek(specMonth[0], date, day, dayNumber[0]);
